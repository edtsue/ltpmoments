...
/* THE AUDIENCE A COLD BOARD OPENS ON.

   The Search target rather than the first of the estimated six. An empty hash
   is somebody arriving with no opinion yet, and the board they are handed
   without asking should rest on the PA's own measured cut rather than on an
   invented placeholder — the first thing anyone sees should be the thing this
   tool can actually defend.

   Held as an id and resolved through the roster rather than as an index, so a
   rebuild of the cut that renames or drops the target falls through to
   something scoreable instead of to `undefined`. The test is what makes the
   fall-through loud. */
const DEFAULT_AUD = 'search26';
const defaultAud = () => {
  const roster = ROSTER();
  return roster.find(a => a.id === DEFAULT_AUD)
    || roster.find(a => !a.pending)
    || roster[0];
};

const S = {
  auds: H.auds || [defaultAud().id],   // one or several; never none
  mode: H.mode || 'blend',
...
function audiences() {
  ...
  /* Recovers to the same audience a cold load opens on, so a dead hash and no
     hash land in the same place — two routes into the tool that disagree about
     where "no selection" goes is a difference nobody can explain. */
  return [defaultAud()];
}
...
function deleteAudience(id) {
  ...
  if (!S.auds.length) S.auds = [defaultAud().id];
  recompute();
  render();
}
...
function boardDigest() {
  ...
  /* THE MODEL, WRITTEN OUT OF THE MODEL.

     The prompt used to carry the scoring rules as prose, and it carried the
     AFFINITY model's rules — the only ones that existed when it was written.
     A reader on the response model was being answered with a formula the
     board was not running and band cuts that were three points out. The
     registry exists so nothing describes a formula it is not executing, and
     the chat rail was the one surface still outside it. */
  const M = MODEL();
  const modelBlock = [
    `HOW THIS BOARD IS SCORED — ${M.label}, out of 100. ${M.gist}`,
    ...M.parts.map(p => `  ${p.name}${p.weight != null ? ' .' + String(Math.round(p.weight * 100)).padStart(2, '0') : (p.note ? ' (' + p.note + ')' : '')} — ${p.q} ${p.why}`),
    `  Bands: ${M.bands.map(b => b.min ? `${b.label} ${b.min}+` : `${b.label} below ${M.bands[M.bands.length - 2].min}`).join(', ')}.`,
    auds.length > 1
      ? `  Several audiences combine on ${M.parts[0].name.toLowerCase()} only, by ${S.mode}: blend is size-weighted, overlap takes the lowest (where one buy serves all), any takes the highest (reach at least one).`
      : `  One audience selected, so nothing is being combined.`
  ];

  /* The panel definition, verbatim, for the audiences that have one. Asked
     "who is the Search audience", the rail's prose says it is the broadest of
     the four; only this says it is Adults 18–34. The digest is also the list
     of figures the reply is allowed to quote, so a filter that is not in here
     comes back flagged as unverified even when it is true. */
  const critLine = a => (a.criteria && a.criteria.length)
    ? `  ${a.name} — panel definition, all of: ${a.criteria.join('; AND ')}`
    : `  ${a.name} — no panel definition; ${a.est ? 'an estimate, not a research cut' : 'defined in words'}`;
  ...
  return [
    ...
    ``,
    `HOW THESE AUDIENCES WERE BUILT:`,
    ...auds.map(critLine),
    ``,
    ...modelBlock,
    ``,
    ...
    off.length ? `CATEGORIES SWITCHED OFF (not on the board, not in this digest): ${off.join(', ')}.`
               /* Counted, not spelled. It said "ten" while the line under it
                  listed twelve — a small wrong number, in the one place whose
                  entire job is to be the only thing the reader can quote. */
               : `All ${CATS.length} categories are on.`,
    ``,
